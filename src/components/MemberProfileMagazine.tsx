...
// Distribute the 0–4 business photos over the layout's three slots:
// a full-bleed spread after the opening section, a two-up pair after the
// achievements section, and a full-bleed spread before the closing section.
function photoSlots(business: string[]) {
  const spread1 = business[0] ?? null;
  let twoUp: [string, string] | null = null;
  let spread2: string | null = null;
  if (business.length === 2) {
    spread2 = business[1];
  } else if (business.length >= 3) {
    twoUp = [business[1], business[2]];
    spread2 = business[3] ?? null;
  }
  return { spread1, twoUp, spread2 };
}

export default async function MemberProfileMagazine({
  member,
  locale,
}: {
  member: Member;
  ...
  const { spread1, twoUp, spread2 } = photoSlots(member.images.business);
  const stats = member.stats ?? [];
  const calloutsAfter = new Map(
    (member.callouts ?? [])
      .filter(
        (c) =>
          sections.some((s) => s.key === c.afterKey) &&
          sections.some((s) => s.key === c.sourceKey)
      )
      .map((c) => [c.afterKey, c])
  );
  const achievementsIdx = sections.findIndex((s) => s.key === "achievements");
  const twoUpAfterIdx = achievementsIdx >= 0 ? achievementsIdx : Math.min(3, Math.max(lastIdx - 1, 0));

  const num = (i: number) => String(i + 1).padStart(2, "0");
  const caption = member.company;
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="px-6 pt-[26px] pb-2 md:px-16 md:pt-16 md:pb-14">
          {sections.map((section, i) => {
            ...
            const callout = calloutsAfter.get(section.key);
            const showStats = section.key === "achievements" && stats.length > 0;
            ...
            return (
              <div key={section.key}>
                ...
                {/* Full-bleed spread after the opening section */}
                {i === 0 && spread1 && (
                  <figure className="mt-8 md:mt-14 md:mb-3">
                    ...
                    <figcaption className={captionText}>{caption}</figcaption>
                  </figure>
                )}

                {/* Two-up photos */}
                {i === twoUpAfterIdx && twoUp && (
            ...
            <div className="mt-7 md:mt-9 border border-[rgba(255,255,255,.14)] px-[22px] py-[26px] md:px-11 md:py-9 md:flex md:items-center md:justify-between md:gap-7">
              <div>
                ...
                <h3 className={`${serif} mt-2.5 mb-1.5 md:mt-3 text-[22px] md:text-[28px] leading-[1.1] font-medium text-bni-warm-white`}>
                  {t("connectCta", { name: connectName(member) })}
                </h3>
